...
public class Heroi extends Entidade{
    ...
    public boolean realizarAcao(Heroi heroi, List<Inimigo> inimigos, Scanner comandoScanner){   /*realiza apenas uma ação */
        ...
        if (comando == 6){  /*passa a vez */
            ...
            System.out.print(getNome() + " usou " + baralho.getCartasNaMao().get(comando - 1).getNome() + ". ");
            if (alvo == 0){
                baralho.getCartasNaMao().get(comando - 1).usar(heroi);
            }else{
                baralho.getCartasNaMao().get(comando - 1).usar(inimigos.get(alvo - 1));
            }
